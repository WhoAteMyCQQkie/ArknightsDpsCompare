...
class Reed(Operator):
	...
	def skill_dps(self, defense, res):
		...
		if self.skill == 2:
			...
			final_atk = self.atk * (1 + atkbuff + self.buff_atk) + self.buff_atk_flat
			hitdmg = np.fmax(final_atk * atk_scale - defense, final_atk * atk_scale * 0.05)
			hitdmgarts = np.fmax(final_atk * atk_scale * self.skill_params[1] * (1-res/100), final_atk * atk_scale * self.skill_params[1] * 0.05)
			dps = (hitdmg + hitdmgarts)/self.atk_interval * (self.attack_speed+aspd)/100
		return dps
...
class Titi(Operator):
	def __init__(self, pp, *args, **kwargs):
		super().__init__("Titi",pp,[1,2,3],[1],3,1,1) #available skills, available modules, default skill, def pot, def mod
		if self.skill == 2: self.talent_dmg = True
		if self.skill_dmg and self.elite > 0: self.name += " vsSleep"
		if not (self.talent_dmg and self.elite > 0): self.name += " vsMoving"
		if self.talent2_dmg and self.elite > 1: self.name += f" >{int(100*self.talent2_params[1])}%hp"
		if self.skill == 3: self.name += " 1awaken/5s"
		if self.targets > 1 and self.skill > 1: self.name += f" {self.targets}targets" ######when op has aoe
